...
router.put('/pregunta', (req, res) => {
  ...
  const pregunta = {
    texto: req.body.pregunta_texto,
    tiempo_respuesta: req.body.tiempo_respuesta
  }
  const id = parseInt(req.body.pregunta_id, 10)
 ...
 * RUTAS DE OPCIONES
 */
//Ruta para modificar una opcion
router.put('/opcion', (req, res) => {
  const sql = 'UPDATE opciones SET ? WHERE id = ?'
  const opcion = {
    texto_opcion: req.body.texto_opcion,
    es_correcta: req.body.es_correcta
  }
  const id = parseInt(req.body.id, 10)
  db.query(sql, [opcion, id], (err, result) => {
    if (err) return res.json(err)
    return res.status(207).json(result)
  })
})
