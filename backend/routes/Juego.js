...
router.get('/partida', async (req, res) => {
	...
	try {
		...
		// Consulta
		const q = query(partidasCollection, where('pin', '==', req.query.pin), where('pregunta_actual', '==', -1))
		...
		if (!querySnapshot.empty) {
			...
			return res.status(207).json({ exists: true })
		} else {
			console.log('Partida no existente')
